...
# 약 이미지로 검색 파트 - server2로 request 요청및 값을 json형식으로 send로 가져와서 리스트 형식으로 변환후 image_to_text.txt 에 값 저장
...
# 약 데이터 중 필요한 데이터만 dictionary로 재배치
...
# 카카오 챗봇 응답 처리 part(챗봇으로 보내는 json 형식으로 변환)
...
#  카카오 챗봇 응답처리 (약 이미지 검색 error response) 
...
# 약국 조회 처리 geo_code(사용자 입력주소 좌표변환), forlium(지도 시각화) 
